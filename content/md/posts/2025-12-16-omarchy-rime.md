{:title "Omarchy 设置中文输入法"
 :layout :post
 :slug "omarchy-rime"
 :draft false
 }

在使用 [Omarchy](https://omarchy.org/) 的第106天，我决定安装中文输入法。
...
没有 Emoji 的输入法是不完整的，我们先安装 [`rime-emoji`](https://github.com/rime/rime-emoji)，然后按照文档里的自助安装的方式配置即可。
...
1. 安装 `fcitx5-rime`，`fcitx5-configtool`，`fcitx5-rime`，`rime-double-pinyin`，`rime-emoji`；
