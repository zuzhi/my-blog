...
Cryogen 默认提供对 Markdown 支持。如果你想使用 AsciiDoc，从你刚创建的博客目录（例如：`my-blog`）下打开 `project.clj`，并更改 `:dependencies` 下，将 `cryogen-markdown` 改为 `cryogen-asciidoc`。现在，编译器将不再从 `content/md` 下查找以 `.md` 结尾的文件，而是在 `content/asc` 目录想查找以 `.asc` 结尾的文件。
...
### Code Syntax Highlighting/代码语法突出显示
